...
# ---------------------------------------------------------------------------
_DARK_INK = "#E0E8E2"
_DARK_INK_LIGHT = "#A8BBAD"
_DARK_GREY = "#7B9485"
_DARK_GRID = "rgba(143,189,143,0.14)"
_DARK_WARM_BG = "#1E2C25"
_DARK_PLOT_BG = "rgba(26,34,32,0.65)"

# ---------------------------------------------------------------------------
# Module-level dark mode toggle — set once per render from app.py
# ---------------------------------------------------------------------------
_dark_mode: bool = False


def set_dark_mode(enabled: bool) -> None:
    """Enable or disable dark mode for all chart output in this render cycle."""
    global _dark_mode
    _dark_mode = enabled
...
LAYOUT_DARK: dict = dict(
    font=dict(family='"Lora", Georgia, serif', size=13, color=_DARK_INK_LIGHT),
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor=_DARK_PLOT_BG,
    hoverlabel=dict(
        bgcolor=_DARK_WARM_BG,
        bordercolor="rgba(143,189,143,0.25)",
        font=dict(family='"Inter", sans-serif', size=12, color=_DARK_INK),
    ),
)


def _layout(**overrides) -> dict:
    """Return LAYOUT_BASE (or dark variant) merged with caller-supplied overrides."""
    base = LAYOUT_DARK if _dark_mode else LAYOUT_BASE
...
def _title(text: str, size: int = 15) -> dict:
    ...
    c = _DARK_INK if _dark_mode else INK_COLOR
    return dict(
        text=text, font=dict(family='"Lora", Georgia, serif', size=size, color=c)
    )


def _xgrid(**extra) -> dict:
    g = _DARK_GRID if _dark_mode else _GRID
    return dict(gridcolor=g, zerolinecolor=g, zerolinewidth=1, **extra)
...
def _ink() -> str:
    """Return the current ink color (title / strong text)."""
    return _DARK_INK if _dark_mode else INK_COLOR


def _ink_lt() -> str:
    """Return the current light ink color (body text / labels)."""
    return _DARK_INK_LIGHT if _dark_mode else INK_LIGHT


def _grey() -> str:
    """Return the current grey color."""
    return _DARK_GREY if _dark_mode else GREY_COLOR


def _warm_bg() -> str:
    """Return the current warm background (hover labels, marker lines)."""
    return _DARK_WARM_BG if _dark_mode else _WARM_BG


def _grid_color() -> str:
    """Return the current grid line color."""
    return _DARK_GRID if _dark_mode else _GRID


def _polar_bg() -> str:
    """Return polar chart background."""
    return "rgba(26,40,34,0.40)" if _dark_mode else "rgba(232,245,233,0.30)"
...
def loss_waterfall(waterfall: dict[str, float], net_kwh: float) -> go.Figure:
    ...
    fig.update_layout(
        **_layout(
            ...
            height=420,
            margin=dict(l=195, r=20, t=55, b=50),
            showlegend=False,
        )
    )
    return fig
...
def monthly_summary(
    ...
    if monthly_yield_opt is not None:
        fig.add_trace(
            go.Bar(
                x=x,
                y=monthly_yield_opt.values,
                name="Optimal orientation",
        ...
        fig.add_trace(
            go.Bar(
                x=x,
                y=monthly_yield.values,
                name="Selected orientation",
                marker_color=seasonal,
                opacity=0.9,
                width=0.4,
                offset=0.2,
            ),
    ...
    fig.update_layout(
        **_layout(
            ...
            xaxis=dict(
                tickmode="array", tickvals=x, ticktext=MONTH_LABELS, **_xgrid()
            ),
            height=360,
            margin=dict(l=65, r=65, t=55, b=50),
            legend=dict(orientation="h", y=1.1, font=dict(size=12, color=_ink())),
            barmode="overlay",
        )
    )
    fig.update_yaxes(
        title_text="Avg daily yield [kWh/day]",
        secondary_y=False,
...
def orientation_heatmap(
    ...
    fig = go.Figure(
        go.Heatmap(
            ...
            colorbar=dict(title="kWh/yr", tickfont=dict(size=12, color=_ink_lt())),
    ...
    fig.update_layout(
        **_layout(
            title=_title(
                f"Annual yield by orientation — best: tilt {opt_tilt}°, facing {opt_az}° → {opt_val:,.0f} kWh/yr"
            ),
            xaxis=dict(
                title="Facing direction [°]  (0° = N · 90° = E · 180° = S · 270° = W)",
                **_xgrid(),
            ),
...
def yield_vs_tilt(
    ...
    fig.add_trace(
        go.Scatter(
            x=tilt_arr,
            y=energy_grid[:, south_j],
            name="South-facing (180°)",
    ...
    fig.add_trace(
        go.Scatter(
            x=[selected_tilt],
            y=[energy_grid[sel_i, sel_j]],
            mode="markers",
            marker=dict(
                size=12, color=BLUE_COLOR, line=dict(color=_warm_bg(), width=1.5)
            ),
            name="Current",
            showlegend=True,
        )
    ...
    fig.update_layout(
        **_layout(
            ...
            xaxis=dict(
                title="Tilt angle [°]  (0° = flat · 90° = vertical)", **_xgrid()
            ),
...
def daily_irradiance(
    ...
    day_mask = (tmy_df.index.month == times[0].month) & (
        tmy_df.index.day == times[0].day
    )
    day_df = tmy_df[day_mask].copy()
    if len(day_df) == 0:
        return go.Figure().update_layout(
            **_layout(title=_title("No data for this day"))
        )
    ...
    date_str = (datetime.date(2023, 1, 1) + datetime.timedelta(days=doy - 1)).strftime(
        "%B %d"
    )
    ...
    fig.add_trace(
        go.Scatter(
            x=hours,
            y=poa["poa_direct"].clip(lower=0),
            name="Direct beam",
            stackgroup="poa",
            fillcolor="rgba(232,146,14,0.72)",
            line=dict(width=0),
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            x=hours,
            y=poa["poa_diffuse"].clip(lower=0),
            name="Sky diffuse",
            stackgroup="poa",
            fillcolor="rgba(74,133,192,0.55)",
            line=dict(width=0),
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            x=hours,
            y=poa["poa_ground_diffuse"].clip(lower=0),
            name="Ground reflected",
            stackgroup="poa",
            fillcolor="rgba(74,122,88,0.45)",
            line=dict(width=0),
        ),
        secondary_y=False,
    )
    ...
    fig.update_layout(
        **_layout(
            ...
            legend=dict(orientation="h", y=1.12, font=dict(size=12, color=_ink())),
        )
    )
    fig.update_yaxes(
        title_text="Solar power on panel [W/m²]",
        secondary_y=False,
        gridcolor=_grid_color(),
        zerolinecolor=_grid_color(),
        color=_ink_lt(),
    ...
    fig.update_yaxes(
        title_text="Sun height above horizon [°]",
        secondary_y=True,
        range=[0, 90],
        showgrid=False,
        color=_grey(),
...
def sun_path_polar(
    lat: float, lon: float, elevation_m: float, selected_doy: int
) -> go.Figure:
    ...
    colors = [WINTER_COLOR, _grey(), SUN_COLOR, TERRACOTTA]
    ...
    fig.update_layout(
        **_layout(
            title=_title(
                "Sun path across the sky — centre = directly overhead, edge = horizon"
            ),
            polar=_polar_style(),
            height=420,
            margin=dict(l=40, r=40, t=65, b=50),
            legend=dict(orientation="h", y=-0.12, font=dict(size=12, color=_ink())),
        )
    )
    return fig
...
def sun_path_flower(
    lat: float, lon: float, elevation_m: float, selected_doy: int | None = None
) -> go.Figure:
    ...
    if selected_doy is not None:
        petals.append(
            ("Selected day", selected_doy, "rgba(199,91,57,0.20)", TERRACOTTA)
        )
    ...
    fig.update_layout(
        **_layout(
            ...
            legend=dict(orientation="h", y=-0.14, font=dict(size=12, color=_ink())),
        )
    )
    return fig
...
def monthly_rose(monthly_yield: pd.Series) -> go.Figure:
    ...
    month_colors = [
        ...
        CLAY,  # Jan, Feb  — winter
        EARTH_COLOR,
        EARTH_COLOR,  # Mar, Apr  — spring
        SUN_COLOR,
        SUN_COLOR,  # May, Jun  — early summer
        SUN_COLOR,
        AMBER_LIGHT,  # Jul, Aug  — peak summer
        TERRACOTTA,
        TERRACOTTA,  # Sep, Oct  — autumn
        CLAY,
        CLAY,  # Nov, Dec  — winter
    ]
        ...
        **_layout(
            ...
            polar=dict(
                ...
                angularaxis=dict(
                    ...
                    direction="clockwise",
                    rotation=90,
                    tickfont=dict(size=13, color=_ink()),
                    gridcolor=_grid_color(),
                ),
                radialaxis=dict(
                    title_text="kWh/day",
                    showticklabels=True,
                    tickfont=dict(size=11, color=_ink_lt()),
                    gridcolor=_grid_color(),
                ),
            ),
            showlegend=False,
...
def energy_roots(waterfall: dict[str, float], net_kwh: float) -> go.Figure:
    ...
    fig.update_layout(
        **_layout(
            title=_title(
                f"Energy flow — {gross:,.0f} kWh gross → {net_kwh:,.0f} kWh net"
            ),
            height=455,
    ...
    fig = go.Figure(
        go.Contour(
            ...
            contours=dict(
                coloring="heatmap",
                showlabels=True,
                labelfont=dict(size=12, color=_ink()),
            ),
            colorbar=dict(title="kWh/yr", tickfont=dict(size=12, color=_ink_lt())),
            hovertemplate="Tilt: %{y}°<br>Azimuth: %{x}°<br>Yield: %{z:.0f} kWh<extra></extra>",
        )
...
def lifetime_yield_chart(lifetime_yield: np.ndarray) -> go.Figure:
    ...
    fig.update_layout(
        **_layout(
            ...
            height=360,
            margin=dict(l=65, r=95, t=55, b=50),
            legend=dict(
                orientation="h",
                yanchor="bottom",
                y=1.02,
...
def horizon_profile_chart(
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            fill="toself",
            fillcolor="rgba(106,127,114,0.22)",
            line=dict(color=_grey(), width=2),
    ...
    fig.update_layout(
        **_layout(
            title=_title(
                "Horizon Profile & Sun Path — shaded area = obstruction zone"
            ),
            polar=_polar_style(),
            height=460,
            margin=dict(l=40, r=40, t=65, b=50),
...
def sensitivity_tornado(
    ...
    """
    Tornado chart: annual yield at low/high extreme of each sensitivity parameter.

    Parameters
    ----------
    sensitivity : dict label → (low_yield_kwh, high_yield_kwh)
    base_yield  : current base annual yield (kWh) — shown as reference line
    """
    # Sort by total swing (largest at top of chart)
    items = sorted(sensitivity.items(), key=lambda x: x[1][1] - x[1][0], reverse=False)
    labels = [k for k, _ in items]
    lows = [v[0] for _, v in items]
    highs = [v[1] for _, v in items]

    # Left bars: range from low_yield up to base (downside)
    low_widths = [max(base_yield - lo, 0.0) for lo in lows]
    low_bases = [min(lo, base_yield) for lo in lows]

    # Right bars: range from base up to high_yield (upside)
    high_widths = [max(hi - base_yield, 0.0) for hi in highs]
    ...
    fig.add_trace(
        go.Bar(
            ...
            x=low_widths,
            base=low_bases,
            customdata=[[lo, (lo - base_yield) / base_yield * 100] for lo in lows],
            orientation="h",
            marker_color=TERRACOTTA,
            marker_opacity=0.85,
            hovertemplate=(
                "<b>%{y}</b><br>"
                "Low: <b>%{customdata[0]:,.0f} kWh</b> "
                "(%{customdata[1]:.1f}% vs base)"
                "<extra></extra>"
            ),
        )
    )
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=labels,
            x=high_widths,
            base=[base_yield] * len(highs),
            customdata=[[hi, (hi - base_yield) / base_yield * 100] for hi in highs],
            orientation="h",
            marker_color=EARTH_COLOR,
            marker_opacity=0.85,
            hovertemplate=(
                "<b>%{y}</b><br>"
                "High: <b>%{customdata[0]:,.0f} kWh</b> "
                "(+%{customdata[1]:.1f}% vs base)"
                "<extra></extra>"
            ),
        )
    ...
    fig.add_vline(
        ...
        line_color=_ink_lt(),
        line_width=2,
        annotation_text=f"Base: {base_yield:,.0f} kWh",
        annotation_position="top",
        annotation_font=dict(family='"Inter", sans-serif', size=11, color=_ink_lt()),
    )

    fig.update_layout(
        **_layout(
            title=_title("What Moves Your Yield Most?", 15),
            barmode="overlay",
            xaxis=dict(title="Annual yield [kWh]", **_xgrid()),
            yaxis=dict(title="", **_ygrid()),
            legend=dict(orientation="h", y=-0.18, x=0.5, xanchor="center"),
            margin=dict(l=200, r=30, t=60, b=70),
            height=60 * len(labels) + 130,
        )
    )
